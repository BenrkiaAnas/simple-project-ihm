...
public class AbstractDao<T> {
    ...
    public List<T> findAll() throws Exception {
        ...
        System.out.println("load(requette)" +load(requette));
    ...
    public Integer loadNexId() throws Exception {
        String requette = "SELECT MAX(id) FROM " + DaoEngigne.transformeToSqlName(entityClass);
        System.out.println("ha rekette ==> " + requette);
        int res = loadOne(requette);
        System.out.println("loadOne(requette) ==> " + res);
        return res;
    ...
    public Integer loadOne(String requette) throws Exception {
        ResultSet resultSet = ConnectDB.load(requette);
        while (resultSet.next()) {
                System.out.println("resultSet.getInt(0)"+ resultSet.getInt(0));
